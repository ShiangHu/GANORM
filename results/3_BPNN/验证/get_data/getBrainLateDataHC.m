...
clear;clc
%% 提取 BrainLateData HC数据



raw_data_path='D:\matlab\long\117配图\data\验证数据\AllData\5_HC\all';
...
%% 简易版所有数据
for i = 1:length(all)
    BrainLata_HC(i).name = all(i).name;
    BrainLata_HC(i).country = all(i).country;
    BrainLata_HC(i).age = all(i).age;
    BrainLata_HC(i).sex = all(i).sex;
    BrainLata_HC(i).freqrange = all(i).freqrange;
    BrainLata_HC(i).dname = all(i).dname;
    BrainLata_HC(i).pre_coh = all(i).pre_coh;
%     all_easy(i).spec = all(i).spec;
 ...
 for i = 1:length(BrainLata_HC)
     BrainLata_HC(i).alphaband = mean(BrainLata_HC(i).pre_coh(:,:,19:29),3);
%      [charpath(i),eglob(i),...
%          cluster(i),eloc(i),modularity(i),...
%          betweenness(i),participation(i)]=cal_network_feature(all_11(i).alphaband(:,:),thr);

     [BrainLata_HC(i).charpath,BrainLata_HC(i).eglob,...
         BrainLata_HC(i).cluster,BrainLata_HC(i).eloc,BrainLata_HC(i).modularity,...
         BrainLata_HC(i).betweenness,BrainLata_HC(i).participation] = cal_network_feature(BrainLata_HC(i).alphaband(:,:),thr);
 end
