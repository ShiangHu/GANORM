...
clear;clc
%% 提取 BrainLateData MS数据

raw_data_path='D:\matlab\long\117配图\data\验证数据\AllData\4_MS\AR';
...
%% 简易版所有数据
for i = 1:length(all)
    BrainLata_MS(i).name = all(i).name;
    BrainLata_MS(i).country = all(i).country;
    BrainLata_MS(i).age = all(i).age;
    BrainLata_MS(i).sex = all(i).sex;
    BrainLata_MS(i).freqrange = all(i).freqrange;
    BrainLata_MS(i).dname = all(i).dname;
    BrainLata_MS(i).pre_coh = all(i).pre_coh;
%     all_easy(i).spec = all(i).spec;
 ...
 for i = 1:length(BrainLata_MS)
     BrainLata_MS(i).alphaband = mean(BrainLata_MS(i).pre_coh(:,:,19:29),3);

     [BrainLata_MS(i).charpath,BrainLata_MS(i).eglob,...
         BrainLata_MS(i).cluster,BrainLata_MS(i).eloc,BrainLata_MS(i).modularity,...
         BrainLata_MS(i).betweenness,BrainLata_MS(i).participation] = cal_network_feature(BrainLata_MS(i).alphaband(:,:),thr);
 end
