...
raw_data_path='D:\matlab\long\117配图\data\验证数据\AllData\2_bvFTD\all';
...
%% 简易版所有数据
for i = 1:length(all)
    BrainLata_bvFTD(i).name = all(i).name;
    BrainLata_bvFTD(i).country = all(i).country;
    BrainLata_bvFTD(i).age = all(i).age;
    BrainLata_bvFTD(i).sex = all(i).sex;
    BrainLata_bvFTD(i).freqrange = all(i).freqrange;
    BrainLata_bvFTD(i).dname = all(i).dname;
    BrainLata_bvFTD(i).pre_coh = all(i).pre_coh;
%     all_easy(i).spec = all(i).spec;
 ...
 for i = 1:length(BrainLata_bvFTD)
     BrainLata_bvFTD(i).alphaband = mean(BrainLata_bvFTD(i).pre_coh(:,:,19:29),3);

     [BrainLata_bvFTD(i).charpath,BrainLata_bvFTD(i).eglob,...
         BrainLata_bvFTD(i).cluster,BrainLata_bvFTD(i).eloc,BrainLata_bvFTD(i).modularity,...
         BrainLata_bvFTD(i).betweenness,BrainLata_bvFTD(i).participation] = cal_network_feature(BrainLata_bvFTD(i).alphaband(:,:),thr);
 end
