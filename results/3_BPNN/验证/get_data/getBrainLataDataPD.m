...
clear;clc
%% 提取 BrainLateData PD数据

raw_data_path='D:\matlab\long\117配图\data\验证数据\AllData\3_PD\all';
...
%% 简易版所有数据
for i = 1:length(all)
    BrainLata_PD(i).name = all(i).name;
    BrainLata_PD(i).country = all(i).country;
    BrainLata_PD(i).age = all(i).age;
    BrainLata_PD(i).sex = all(i).sex;
    BrainLata_PD(i).freqrange = all(i).freqrange;
    BrainLata_PD(i).dname = all(i).dname;
    BrainLata_PD(i).pre_coh = all(i).pre_coh;
%     all_easy(i).spec = all(i).spec;
 ...
 for i = 1:length(BrainLata_PD)
     BrainLata_PD(i).alphaband = mean(BrainLata_PD(i).pre_coh(:,:,19:29),3);

     [BrainLata_PD(i).charpath,BrainLata_PD(i).eglob,...
         BrainLata_PD(i).cluster,BrainLata_PD(i).eloc,BrainLata_PD(i).modularity,...
         BrainLata_PD(i).betweenness,BrainLata_PD(i).participation] = cal_network_feature(BrainLata_PD(i).alphaband(:,:),thr);
 end
